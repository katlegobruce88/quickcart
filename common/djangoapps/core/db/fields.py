"""Custom fields for handling JSON sanitization, Money, and TaxedMoney."""
import json
...
class DecimalMoneyValue:
    """
    Temporary replacement for Money class from prices library.
    Simple class that holds a decimal amount and currency code.
    """
    ...
    def __repr__(self):
        return f"DecimalMoneyValue({self.amount!r}, {self.currency!r})"
...
class NonDatabaseFieldBase:
    ...
    def contribute_to_class(self, cls, name, **kwargs):
        # kwargs is part of Django's field interface but not used in this
        # implementation
    ...
    def clean(self, value, model_instance):  # pylint: disable=unused-argument
        # Shortcircut clean() because Django calls it on all fields with
        # is_relation = False
        # model_instance argument is required by Django Field interface but not
        # used here
        return value
...
class MoneyField(NonDatabaseFieldBase):
    ...
    def __str__(self):
        return (
            f"MoneyField(amount_field={self.amount_field}, "
            f"currency_field={self.currency_field})"
        )
    ...
    def get_default(self):
        ...
        if hasattr(self, "model"):
            ...
            default_amount = (
                self.model._meta.get_field(self.amount_field).get_default()
            )
...
class TaxedMoneyField(NonDatabaseFieldBase):
    ...
    def __str__(self):
        return (
            f"TaxedMoneyField(net_amount_field={self.net_amount_field}, "
            f"gross_amount_field={self.gross_amount_field}, "
            f"currency_field={self.currency_field})"
        )
